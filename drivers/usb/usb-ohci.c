 ...
 * [ (C) Copyright 2000 Masato Abe]
 * 
 * 
 * History:
 * 
 * v5.2b2001/ 1/ 9 PS2(based on 2.2.18)
 * v5.2a2000/ 4/24 PS2
...
#ifdef CONFIG_PS2
extern void *ps2sif_allociopheap(int);
extern int ps2sif_freeiopheap(void *);
extern unsigned long ps2sif_virttobus(volatile void *a);
extern void * ps2sif_bustovirt(unsigned long a);
...
#ifdef CONFIG_PS2
	if (urb_priv->transfer_buffer)
		FreeLocalMem(urb_priv->transfer_buffer);
	if (urb_priv->setup_packet)
		FreeLocalMem(urb_priv->setup_packet);
#endif
	kfree (urb->hcpriv);
	urb->hcpriv = NULL;
...
static int sohci_return_urb (urb_t * urb)
{
	...
	if (urb->transfer_buffer_length) {
		memcpy(urb->transfer_buffer, urb_priv->transfer_buffer, urb->transfer_buffer_length);
	}
#endif
...
//		return -EPIPE;
	...
	for (i = 0; i < size; i++) { 
#ifdef CONFIG_PS2
		urb_priv->td[i] = (td_t *)AllocLocalMem(sizeof(td_t));
#else
...
#ifdef CONFIG_PS2
	if (urb->transfer_buffer_length) {
		urb_priv->transfer_buffer = (char *)AllocLocalMem(urb->transfer_buffer_length);
		if (!urb_priv->transfer_buffer) {
			usb_dec_dev_use (urb->dev);	
			urb_rm_priv (urb);
			spin_unlock_irqrestore (&usb_ed_lock, flags);
		...
		memcpy(urb_priv->transfer_buffer, urb->transfer_buffer,
				urb->transfer_buffer_length);
	}
	if (urb->setup_packet) {
		urb_priv->setup_packet = (char *)AllocLocalMem(8);
		if (!urb_priv->setup_packet) {
			usb_dec_dev_use (urb->dev);	
			urb_rm_priv (urb);
			spin_unlock_irqrestore (&usb_ed_lock, flags);
		...
		memcpy(urb_priv->setup_packet, urb->setup_packet, 8);
	}
#endif
...
#ifdef CONFIG_PS2
	dev = (struct ohci_device *)ps2sif_allociopheap(sizeof(struct ohci_device));
	if (!dev)
		return -ENOMEM;
	dev = (struct ohci_device *)bus_to_virt((unsigned)dev);
#else
...
#ifdef CONFIG_PS2
        ps2sif_freeiopheap((void *)virt_to_bus(dev));
#else
	kfree (dev);
#endif
...
#ifdef CONFIG_PS2
		td = (td_t *)AllocLocalMem(sizeof(*td));
#else
...
#ifdef CONFIG_PS2
	void * ctrl = urb_priv->setup_packet;
	void * data = urb_priv->transfer_buffer;
#else
...
#ifdef CONFIG_PS2
 				if (td->hwCBP == 0)
					urb->actual_length = bus_to_virt (tdBE) - urb_priv->transfer_buffer + 1;
  				else
					urb->actual_length = bus_to_virt (tdCBP) - urb_priv->transfer_buffer;
#else
 				if (td->hwCBP == 0)
...
static ohci_t * hc_alloc_ohci (void * mem_base)
{
	...
	ohci = (ohci_t *) ps2sif_allociopheap(sizeof(ohci_t));
	if (!ohci) {
		CleanupLocalMem();
		kfree(FreeList);
		return NULL;
	}
	ohci = (ohci_t *)bus_to_virt((unsigned) ohci);
#else
	...
	if (!bus) {
#ifdef CONFIG_PS2
		ps2sif_freeiopheap((void *)virt_to_bus(ohci));
		CleanupLocalMem();
		kfree(FreeList);
#else
...
{
#ifdef CONFIG_PS2
	return hc_found_ohci (dev, IRQ_SBUS_USB, (void *)0x1f801600);
#else
