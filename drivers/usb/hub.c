...
#ifdef CONFIG_PS2
static void usb_hub_port_power_on(struct usb_hub *hub, int port)
{
	/* Enable power to the specified port */
	dbg("enabling power on the specified port");
	usb_set_port_feature(hub->dev, port + 1, USB_PORT_FEAT_POWER);
}
#endif
